...
function atualizaDisplay(calculadora) {
  // Atualiza o buffer (parte de cima da tela)
  if (calculadora.operador !== "") {
    calculadora.bufferTextoElemento.textContent =
      calculadora.operandoAnterior + " " + calculadora.operador;
  } else {
    calculadora.bufferTextoElemento.textContent = "";
  }
  // Atualiza o display principal (parte de baixo da tela)
...
function executaCalculo(calculadora) {
  let anterior = Number(calculadora.operandoAnterior);
  let atual = Number(calculadora.operandoAtual);
  let resultado = "";
